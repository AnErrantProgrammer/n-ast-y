<template>
  <div id="app">
    <b-modal id="teacher" title="Program Teacher">
      <b-form-textarea v-model="teacherProgram" rows="6" max-rows="12"></b-form-textarea>
    ...
    <b-modal id="select-validation" title="Sample Validations">
      ...
      <ul class="list-unstyled">
        <span
          >Selected Validation: <strong>{{ selectedValidation.name }}</strong></span
        >
        <hr />
        <li v-for="(validation, index) in sampleValidations" v-bind:key="index">
          <a
            ...
            "
            >{{ validation.name }} - {{ validation.description }}</a
          >
        </li>
      </ul>
    </b-modal>
        ...
        <b-col cols="9">
          <b-row>
            <b-col cols="6">
              <b-card no-body bg-variant="light">
                <template v-slot:header>
                  <div class="mb-0">
                    <h6 style="display:inline" class="mb-0">Current Generations Programs</h6>
                    <span style="float:right" class="ml-auto">
                      <div class="text-center">
                        <b-button variant="light" size="sm">
                          Generation # <b-badge variant="primary">{{ totalGenerations }}</b-badge>
                        </b-button>
                      </div>
                    </span>
                  </div>
                ...
                <b-card-body>
                  <b-card-text>
                    <pre style="height:243px;overflow-y: scroll;">{{ selectedProgram() }}</pre>
                  </b-card-text>
                </b-card-body>

                <b-card-footer>
                  <b-button size="sm" class="mr-1" variant="info" v-bind:disabled="programs.length==0"  @click="prevProgram">Previous</b-button>
                  {{ selectedProgramIndex }} of {{programs.length}}
                  <b-button size="sm" class="mr-1" variant="info" v-bind:disabled="programs.length==0"  @click="nextProgram">Next</b-button>
                </b-card-footer>
              </b-card>
            </b-col>
            ...
            <b-col cols="6">
              <b-card bg-variant="light">
                <template v-slot:header>
                  <div class="mb-0">
                    <h6 style="display:inline" class="mb-0">Fittest Program</h6>
                    <span style="float:right" class="ml-auto">
                      <div class="text-center">
                        <b-button variant="light" size="sm">
          ...
          <b-row style="margin-top:25px">
            <b-col cols="6">
              ...
              <template v-slot:header>
                  <div class="mb-0">
                    <h6 style="display:inline" class="mb-0">Log</h6>
                  </div>
                </template>
                <b-card-text>
                  <div id="log" style="height:230px;overflow-y: scroll;"></div>
                </b-card-text>
              </b-card>
            </b-col>
            <b-col cols="6">
              ...
              <template v-slot:header>
                  <div class="mb-0">
                    <h6 style="display:inline" class="mb-0">Test Part</h6>
                  </div>
                </template>
                <b-card-text>
                <b-form inline>
                  <b-form-select v-model="testPart">
                    <option v-for="(partWeight, key) in partWeights" v-bind:key="key" v-bind:value="key">{{ key }}</option>
                  </b-form-select>
                  &nbsp;&nbsp;
                  <b-button size="sm" class="mr-1" v-bind:disabled="testPart==''" variant="info" @click="generateNode">Generate Node</b-button>
                  </b-form>
                  <hr/>
                  <pre style="height:143px;overflow-y: scroll;">{{ testNode }}</pre>
                </b-card-text>
              </b-card>
            </b-col>
          </b-row>
          ...
          <b-tabs>
            <b-tab title="Settings">
              <ul class="list-unstyled sidebar-list">
                ...
                <li>
                  <p>
                    Selected Validation: <strong>{{ selectedValidation.name }}</strong>
                  </p>
            ...
            <b-tab title="Properties">
              <ul class="list-unstyled sidebar-list">
                <li v-for="(propertyWeight, key) in propertyWeights" v-bind:key="key">
                  <a v-bind:name="key"><strong v-if="Object.keys(propertyWeight).length > 0">{{ key }}</strong></a>
                  <ul class="list-unstyled" v-if="Object.keys(propertyWeight).length > 0">
                    <li v-for="(propertyWeightKey, key) in propertyWeight" v-bind:key="key">
                      <small
                        ><strong>{{ key }}</strong></small
                      >
                      <ul class="list-unstyled">
                        <li v-for="(part, index) in propertyWeightKey[0]" v-bind:key="index">
                          <b-row>
                            <b-col cols="8">
                              <label><a v-bind:href="'#' + part">{{ part }}</a></label>
                            </b-col>
                            <b-col cols="3">
...
import ArgumentWeights from "./language/Arguments.js";
...
export default {
  ...
  data: function() {
    return {
      ...
      argumentWeights: ArgumentWeights,
  ...
  methods: {
    ...
    generateNode() {
      let program = this.createNode(this.testPart, null);
          ...
          if (action == "mutate") {
            ...
            if (syntaxPart.type === "Static") {
              ...
              if (node.callee.type == "Identifier") {
                let identifier = node.callee.name;
    ...
    createNode(keyName, parent, depth = 0) {
      ...
      // For the given syntactic part select one of the valid types
      var key = vm.selectedWeightedPart(keyName);

      // Create an empty scope or inherit the scope from the parent
      scope = vm.initializeScope(parent);

      // The node parts is an object which represents the abstract syntax tree for this node
      let nodeParts = {
        scope: scope,
        parent: parent,
      };
        ...
        try {
          ...
          if (syntaxPart.type === "Static") {
            ...
            if (nodeParts.callee.type == "Identifier") {
              let identifier = nodeParts.callee.name;
              if (typeof vm.argumentWeights[identifier] !== "undefined") {
                argumentPattern = chance.weighted(vm.argumentWeights[identifier][0], vm.argumentWeights[identifier][1]);
              }
            } else if (nodeParts.callee.type == "MemberExpression") {
              let obj = nodeParts.callee.object.name;
              let property = nodeParts.callee.property.name;
              if (typeof vm.argumentWeights[obj] != "undefined" && typeof vm.argumentWeights[obj][property] !== "undefined") {
                argumentPattern = chance.weighted(vm.argumentWeights[obj][property][0], vm.argumentWeights[obj][property][1]);
              }
            }
